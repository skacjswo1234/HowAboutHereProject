...
<div class="container">
	<form action="#"  onsubmit="updateAfter()">
		<div class="form-group">
			<label for="title">title:</label> 
			<input id="title" value="${afterEntity.title}" type="text" class="form-control" placeholder="Enter title" name="title" required="required" />
		</div>
		<div class="form-group">
	...
	<script>
		async function updateAfter(){ 
			...
			let response = await fetch("/after/${afterEntity.id}", {
				method: "put",
			...
			if(parseResponse === "ok") { // ==값 비교 , === 타입, 값을 같이 비교한다
				...
				location.href ="/after/${afterEntity.id}";
			}		
		}
